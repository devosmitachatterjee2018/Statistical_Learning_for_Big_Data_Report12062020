...
#%%
# Find best hyperparameters for logistic regression model
solver = ['saga', 'liblinear']
penalty = ['l1']
C = [ 0.001, 0.01, 0.1, 1]
# Define grid search
grid = dict(solver=solver,penalty=penalty,C=C)
bestparameterslogisticregressionmodel = GridSearchCV(estimator=LogisticRegression(random_state=0), param_grid=grid,
                    verbose=1,
                    n_jobs=-1)
...
# Define model
logisticregressionmodel = LogisticRegression(random_state=0, penalty='l1', C=1, solver='liblinear')
# Fit the model
...
all_sample_title = 'Confusion matrix for l1-logistic regression\n Accuracy Score: {0}'.format(accuracyscore)
plt.title(all_sample_title, fontsize = 18)
plt.savefig('plot_beforeselection_l1_confusionmatrix.png')
...
plt.figure(figsize=(10,6))
plt.plot(fpr, tpr, label='L1-logistic regression (AUC = %f)' % logit_roc_auc)
plt.plot([0, 1], [0, 1],'r--')
plt.xlim([0.0, 1.0])
plt.ylim([0.0, 1.05])
plt.xlabel('False positive rate', fontsize = 12)
plt.ylabel('True positive rate', fontsize = 12)
all_sample_title = 'ROC curve for l1-logistic regression\n ROC AUC Score: {0}'.format(logit_roc_auc)
plt.title(all_sample_title, fontsize = 18)
plt.legend(loc="lower right")
plt.savefig('plot_beforeselection_l1_ROCcurve.png')
plt.show()
...
plt.title('Bar chart of l1-logistic regression coefficients as feature importance scores', fontsize=18)
plt.savefig('plot_l1_featureimportancescores.png')
plt.show()
...
all_sample_title = 'Confusion matrix for l1-logistic regression\n Accuracy Score: {0}'.format(accuracyscore)
plt.title(all_sample_title, fontsize = 18)
plt.savefig('plot_afterselection_l1_confusionmatrix.png')
...
plt.title(all_sample_title, fontsize = 18)
plt.legend(loc="lower right")
plt.savefig('plot_afterselection_l1_ROCcurve.png')
plt.show()
